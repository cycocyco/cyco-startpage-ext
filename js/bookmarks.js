...
(function () {
  ...
  async function renderBookmarks(newId, bust) {
    ...
    btn.hidden = list.length <= 6;
    wrap.classList.toggle("collapsed", list.length > 6 && !btn.dataset.expanded);
    wrap.classList.toggle("expanded", list.length > 6 && btn.dataset.expanded);
  }
